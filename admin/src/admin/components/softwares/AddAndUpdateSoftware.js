...
const AddAndUpdateSoftware = () => {
  ...
  const formHandlerUpdate = (e) => {
    ...
    axios.put("http://localhost:5000/api/software-management/", formData)
      .then((response) => {
  ...
  const formHandler = (e) => {
    ...
    axios
      .post("http://localhost:5000/api/software-management/", formData)
      .then((response) => {
        if (response.data.success) {
  ...
  useEffect(() => {
    if (id) {
      ...
      axios.get(
        "http://localhost:5000/api/software-management/fetch/",
        { params: { id: id } },
