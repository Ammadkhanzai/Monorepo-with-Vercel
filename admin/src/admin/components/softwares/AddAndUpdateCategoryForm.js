...
const AddAndUpdateCategoryForm = (props) => {
  ...
  const addCategory = () => {
    ...
    axios.post('http://localhost:5000/api/category/', { categoryName })
      .then(response => {
  ...
  const updateCategory = () => {
    ...
    axios.put('http://localhost:5000/api/category/', { id, categoryName })
      .then(response => {
        if (response.status === 201) {
          setMessage({ status: true, color: "green", text: response.data.message })
  ...
  useEffect(() => {
    ...
    if (props.updateId.id) {
      axios.get(`http://localhost:5000/api/category/${props.updateId.id}`)
        .then(response => {
