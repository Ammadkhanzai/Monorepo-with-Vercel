...
const Softwares = () => {
  ...
  const test = (id) => {
    axios.get('/api/latest-software/single/' + id)
      .then((response) => {
  ...
  const addToPopular = (id) => {
    // console.log(id)
    axios.post("/api/popular-software/", { softwareID: id })
      .then((response) =>
  ...
  const addToLatest = (id) => {
    console.log(id)
    axios.post("/api/latest-software/", { softwareID: id })
      .then((response) =>
        console.log(response)
      )
      .catch((err) =>
  ...
  function deleteSoftware(id) {
    ...
    params.append("id", id);
    axios.delete("/api/software-management/", { params })
      .then((response) => {
        if (response.data.success) {
          axios.delete("/api/latest-software/column/", { params })
          axios.delete("/api/popular-software/column/", { params })
          setSoftware({
  ...
  useEffect(() => {
    ...
    axios.get("/api/software-management/", { cancelToken: cancelTokenSource.token })
      .then((response) => {
        if (response.data.success) {
